   ...
   if (count > 0) {
      switch (elementSize) {
         case 1:
            ...
            break;
         case 2:
            rc = BDMCF_CMD_WRITE_MEM_W(addr, pack16BE(data_ptr));
            count >>= 1;
            count--;
            while ((count > 0) && (rc == BDM_RC_OK)) {
               data_ptr       += 2;
               addr           += 2;
               count--;
               rc = BDMCF_CMD_FILL_MEM_W(pack16BE(data_ptr));
            }
            break;
         case 4:
            rc = BDMCF_CMD_WRITE_MEM_L(addr, pack32BE(data_ptr));
            count >>= 2;
            count--;
            while ((count > 0) && (rc == BDM_RC_OK)) {
               data_ptr       += 4;
               addr           += 4;
               count--;
               rc = BDMCF_CMD_FILL_MEM_L(pack32BE(data_ptr));
            }
            break;
         default:
   ...
   if (count >0) {
      switch (elementSize) {
         case 1:
            ...
            break;
         case 2:
            rc = BDMCF_CMD_READ_MEM_W(addr, data_ptr);
            count >>= 1;
            count--;
            while ((count > 0) && (rc == BDM_RC_OK)) {
               data_ptr       += 2;
               addr           += 2;
               count--;
               rc = BDMCF_CMD_DUMP_MEM_W(data_ptr);
            }
            break;
         case 4:
            rc = BDMCF_CMD_READ_MEM_L(addr, data_ptr);
            count >>= 2;
            count--;
            while ((count > 0) && (rc == BDM_RC_OK)) {
               data_ptr       += 4;
               addr           += 4;
               count--;
               rc = BDMCF_CMD_DUMP_MEM_L(data_ptr);
            }
            break;
         default:
            return BDM_RC_ILLEGAL_PARAMS;
...
USBDM_ErrorCode f_CMD_READ_REG(void) {
   ...
   returnSize = 5;
   return BDMCF_CMD_READ_REG(commandBuffer[3]&0x9F, commandBuffer+1);
}
...
USBDM_ErrorCode f_CMD_READ_ALL_CORE_REGS(void) {
   ...
   while (regIndex<=endRegister) {
      ...
      rc = BDMCF_CMD_READ_REG(regIndexMap[regIndex], outputPtr);
      if (rc != BDM_RC_OK) {
         return rc;
      }
      outputPtr  += 4;
...
USBDM_ErrorCode rc = BDM_RC_OK;
   ...
   if (regNo >= CFV1_ByteRegs) {
      ...
      rc = BDMCF_CMD_READ_DREG((uint8_t)regNo&0x1F, commandBuffer+1);
...
USBDM_ErrorCode f_CMD_READ_CREG(void) {
   returnSize = 5;
   return BDMCF_CMD_READ_CREG(commandBuffer[3]&0x1F, commandBuffer+1);
}
