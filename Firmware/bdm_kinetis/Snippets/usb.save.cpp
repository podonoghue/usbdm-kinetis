...
static void ep1InitialiseBdtRx();
static void resetDeviceCallback();
...
//
constexpr uint  CONTROL_EP_MAXSIZE      = 32; //!< USBDM - Control in/out   	           32
...
constexpr uint  CDC_DATA_IN_EP_MAXSIZE  = 16; //!< USBDM - CDC data in (pin-pong)   x2 = 32
#endif
...
// BDM
uint8_t bdmOutDataBuffer[BDM_OUT_EP_MAXSIZE];          //!< OUT,bulk   - BDM Command & data out
uint8_t bdmInDataBuffer[BDM_IN_EP_MAXSIZE];            //!< IN, bulk   - BDM data in
...
static const uint8_t *const stringDescriptors[] = {
      s_language, s_manufacturer, s_product, s_serial,
      s_bdm_interface,
#if (HW_CAPABILITY&CAP_CDC)
      s_cdc_interface, s_cdc_control, s_cdc_data
#endif
};
 ...
 */
static const struct Descriptors {
...
} otherDescriptors =
{
      ...
      { // bdm_out_endpoint - OUT,Bulk
            ...
            /* bEndpointAddress        */ EP_OUT|BDM_OUT_ENDPOINT,
      ...
      { // bdm_in_endpoint - IN, BULK
            /* bLength                 */ sizeof(EndpointDescriptor),
            /* bDescriptorType         */ DT_ENDPOINT,
            /* bEndpointAddress        */ EP_IN|BDM_IN_ENDPOINT,
            /* bmAttributes            */ ATTR_BULK,
...
struct EPState {
   ...
   void    (*volatile  callback)();           //!< Callback used on completion of packet reception
};
...
static volatile EPState ep0State = {nullptr, 0, 0, 0, nullptr};
static volatile EPState ep1State = {nullptr, 0, 0, 0, nullptr};
static volatile EPState ep2State = {nullptr, 0, 0, 0, nullptr};

static EPHardwareState epHardwareState[NUMBER_OF_ENDPOINTS] = {{0,0,0,EPIdle}};

// Activity
template <class Info>
bool UsbBase_T<Info>::activityFlag;

/**
 * Buffer for EP0 Setup packet (copied from USB RAM)
 */
template <class Info>
SetupPacket UsbBase_T<Info>::ep0SetupBuffer;   //!< Buffer for EP0 Setup packet (copied from USB RAM)
 ...
 */
template<class Info>void UsbBase_T<Info>::ep0StartRxTransaction( uint8_t bufSize, uint8_t *bufPtr ) {
 ...
 */
template<class Info>void UsbBase_T<Info>::ep0InitialiseBdtRx( uint8_t data0_1 ) {
   // Set up to Rx packet
 ...
 */
template<class Info>uint8_t UsbBase_T<Info>::ep0SaveRxData() {
   // Set up to Rx packet
   BdtEntry *bdt = (!epHardwareState[CONTROL_ENDPOINT].rxOdd)?&endPointBdts[CONTROL_ENDPOINT].rxOdd:&endPointBdts[CONTROL_ENDPOINT].rxEven;
 ...
 */
template<class Info> void UsbBase_T<Info>::ep0ConfigureSetupTransaction() {
 ...
 */
template<class Info>void UsbBase_T<Info>::ep0EnsureReadyForSetupTransaction() {
 ...
 */
template<class Info>void UsbBase_T<Info>::ep0InitialiseBdtTx() {
...
template<class Info>
void UsbBase_T<Info>::initialiseEndpoints(void) {
   ...
   // Enable end-points
   usb->ENDPOINT[CONTROL_ENDPOINT].ENDPT      = USB_ENDPT_EPRXEN_MASK|USB_ENDPT_EPTXEN_MASK|USB_ENDPT_EPHSHK_MASK; // Rx/Tx/SETUP
   usb->ENDPOINT[BDM_OUT_ENDPOINT].ENDPT      = USB_ENDPT_EPRXEN_MASK|                      USB_ENDPT_EPHSHK_MASK; // Rx
   usb->ENDPOINT[BDM_IN_ENDPOINT].ENDPT       =                       USB_ENDPT_EPTXEN_MASK|USB_ENDPT_EPHSHK_MASK; // Tx
#if (HW_CAPABILITY&CAP_CDC)
   usb->ENDPOINT[CDC_CONTROL_ENDPOINT].ENDPT  =                       USB_ENDPT_EPTXEN_MASK|USB_ENDPT_EPHSHK_MASK; // Tx
   usb->ENDPOINT[CDC_DATA_OUT_ENDPOINT].ENDPT = USB_ENDPT_EPRXEN_MASK|                      USB_ENDPT_EPHSHK_MASK; // Rx
   usb->ENDPOINT[CDC_DATA_IN_ENDPOINT].ENDPT  =                       USB_ENDPT_EPTXEN_MASK|USB_ENDPT_EPHSHK_MASK; // Tx
#endif
}
 ...
 */
template<class Info> void UsbBase_T<Info>::epStall(uint8_t epNum) {
   if (epNum == CONTROL_ENDPOINT) {
      ...
      usb->ENDPOINT[epNum].ENDPT    |= USB_ENDPT_EPSTALL_MASK;
   }
}
 ...
 *  @param epNum - endpoint number
 */
template<class Info> void UsbBase_T<Info>::epClearStall(uint8_t epNum) {
   usb->ENDPOINT[epNum].ENDPT          &= ~USB_ENDPT_EPSTALL_MASK;
 ...
 */
template<class Info> void UsbBase_T<Info>::setUSBdefaultState() {
   UsbLed::off();
 ...
 */
template<class Info> void UsbBase_T<Info>::setUSBaddressedState( uint8_t address ) {
   if (address == 0) {
      // Unaddress??
      setUSBdefaultState();
   }
 ...
 */
template<class Info> void UsbBase_T<Info>::setUSBconfiguredState( uint8_t config ){
   if (config == 0) {
      // unconfigure
 ...
 */
template<class Info> void UsbBase_T<Info>::ep0StartTxTransaction( uint16_t bufSize, const uint8_t *bufPtr ) {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleGetLineCoding() {
   //   PUTS("getLineCoding");
 ...
 */
template<class Info>void  UsbBase_T<Info>::setLineCodingCallback() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetLineCoding() {
   //   PUTS("setLineCoding");
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetControlLineState() {
 ...
 * Send BREAK on CDC
 */
template<class Info>void  UsbBase_T<Info>::handleSendBreak() {
   //   PUTS("sendBreak");
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleGetStatus() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleClearFeature() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetFeature() {
   bool okResponse = false;
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleGetDescriptor() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::setAddressCallback() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetAddress() {
   //   PUTS("setAddress - ");
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleGetConfiguration() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetConfiguration() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetInterface() {
   //   PUTS("setInterface");
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleGetInterface() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSetupToken( ) {
   //   PUTS("ep0SetupTok");
 ...
 * ep0 - IN
 */
template<class Info>void  UsbBase_T<Info>::ep0HandleInToken() {
   ...
   case EPDataIn:    // Doing a sequence of IN packets (until data count <= EPSIZE)
      ...
      //      uint8_t *p = (uint8_t*)(bdts[usb->STAT>>2].addr);
      //      for(int i=0; i<bdts[usb->STAT>>2].bc; i++) {
      //         PRINTF("%2.2X ", *p++);
      //      }
      //      PUTS("");
      //   }
 ...
 * ep0 - OUT
 */
template<class Info>void  UsbBase_T<Info>::ep0HandleOutToken() {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleUnexpected() {
   //   PUTS("unexpected");
   epStall(0);
}
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleStallComplete() {
   //   PUTS("StallComplete");
   epClearStall(0);
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleSOFToken() {
   // Activity LED
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleUSBReset(void) {
   PUTS("\nReset");
 ...
 */
template<class Info>bool UsbBase_T<Info>::handleTokenComplete(void) {
   // Status from Token
   ...
   // Direction of T/F 0=>OUT, (!=0)=>IN
   uint8_t   isTx     = usbStat&USB_STAT_TX_MASK;

   // Odd/even buffer
   uint8_t   isOdd    = usbStat&USB_STAT_ODD_MASK;
   ...
   // Control - Accept IN, OUT or SETUP token
#if 0
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleUSBSuspend() {
   //   PUTS("Suspend");
   if (deviceState.state != USBconfigured) {
 ...
 */
template<class Info>void  UsbBase_T<Info>::handleUSBResume() {
   //   PUTS("Resume");
 ...
 */
static void ep2InitialiseBdtTx() {
   // Set up to Rx packet
...
template<int ENDPOINT_NUM, int EP_MAXSIZE>
class InEndpoint {
public:
   static volatile EPState state;
   static uint8_t          bdmInDataBuffer[EP_MAXSIZE];
   static EPHardwareState  epHardwareState;

   InEndpoint() {
      static const EPState          initialState = {nullptr, 0, 0, 0, nullptr};
      static const EPHardwareState  initialHardwareState = {0,0,0,EPIdle};
      state           = initialState;
      epHardwareState = initialHardwareState;
   }
   /**
    * Configure the BDT for In [Tx, device -> host]
    */
   static void initialiseBdtTx() {
      // Set up to Rx packet
      BdtEntry *bdt = epHardwareState.txOdd?&endPointBdts[ENDPOINT_NUM].txOdd:&endPointBdts[ENDPOINT_NUM].txEven;

      uint16_t size = state.dataRemaining;
      if (size > EP_MAXSIZE) {
         size = EP_MAXSIZE;
      }
      // Copy the Tx data to EP buffer
      (void) memcpy(bdmInDataBuffer, state.dataPtr, size);

      state.dataPtr         += size;  // Ptr to _next_ data
      state.dataRemaining   -= size;  // Count of remaining bytes
      state.dataCount       += size;  // Count of bytes so far

      // Set up to Tx packet
      bdt->bc     = (uint8_t)size;
      if (epHardwareState[ENDPOINT_NUM].data0_1) {
         bdt->u.bits  = BDTEntry_OWN_MASK|BDTEntry_DATA1_MASK|BDTEntry_DTS_MASK;
      }
      else {
         bdt->u.bits  = BDTEntry_OWN_MASK|BDTEntry_DATA0_MASK|BDTEntry_DTS_MASK;
   ...
   static void startTxTransaction( uint8_t bufSize, const uint8_t *bufPtr ) {
      ...
      state.dataPtr            = (uint8_t*)bufPtr;    // Pointer to _next_ data
      state.dataRemaining      = bufSize;             // Count of remaining bytes
      state.dataCount          = 0;                   // Reset count of bytes so far

      // Note - Always terminate transfers with a truncated/zero packet
      if (bufSize < EP_MAXSIZE) {         // Undersize packet
         epHardwareState[ENDPOINT_NUM].state = EPLastIn;    // Sending one and only packet
      }
      else {
         epHardwareState[ENDPOINT_NUM].state = EPDataIn;    // Sending first of several packets
      }
   ...
   static void handleInToken() {
      ...
      epHardwareState[ENDPOINT_NUM].data0_1 = !epHardwareState[ENDPOINT_NUM].data0_1;   // Toggle DATA0/1 for next packet
      //   PUTS(epHardwareState[BDM_OUT_ENDPOINT].data0_1?"ep2HandleInToken-T-1\n":"ep2HandleInToken-T-0\n");

      switch (epHardwareState[ENDPOINT_NUM].state) {
      case EPDataIn:    // Doing a sequence of IN packets (until data count <= EPSIZE)
         if (ep2State.dataRemaining < BDM_IN_EP_MAXSIZE) {
            epHardwareState[ENDPOINT_NUM].state = EPLastIn;    // Sending last packet (may be empty)
         }
         else {
            epHardwareState[ENDPOINT_NUM].state = EPDataIn;    // Sending full packet
         }
      ...
      case EPLastIn:    // Just done the last IN packet
         if (commandBusyFlag) {
            ...
            epHardwareState[ENDPOINT_NUM].state = EPIdle;  // No more transactions expected
         }
         break;
...
InEndpoint bdmInEndpoint<BDM_IN_ENDPOINT, BDM_IN_EP_MAXSIZE>();
...
void setBDMBusy(void) {
   ...
   if (epHardwareState[BDM_IN_ENDPOINT].state == EPIdle) {
...
/**
 * Handler for Token Complete USB interrupt for other than EP0
 *
 * Handles ep1 [Out]
 * Handles ep2 [In]
 * Handles ep3 [In]
...
void Usb0::handleTokenComplete(void) {
   ...
   if (UsbBase_T<Usb0Info>::handleTokenComplete()) {
      // For EP0 - done
      return;
   }
   switch (endPoint) {
