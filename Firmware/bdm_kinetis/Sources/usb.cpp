...
//
#if (HW_CAPABILITY&CAP_CDC)
constexpr uint  CONTROL_EP_MAXSIZE      = 32; //!< USBDM - Control in/out   	         16
...
static const uint8_t *const stringDescriptors[] = {
      s_language, s_manufacturer, s_product, s_serial, s_bdm_interface,
#if (HW_CAPABILITY&CAP_CDC)
...
void Usb0::initialise() {
   ...
   USB0->CTL      = 0;
   USB0->ADDR     = 0;
   for (unsigned i=0; i<(sizeof(USB0->ENDPOINT)/sizeof(USB0->ENDPOINT[CONTROL_ENDPOINT])); i++) {
 ...
 */
void Usb0::handleStallComplete( void ) {
   //   PUTS("StallComplete");
   epClearStall(0);
 ...
 */
void Usb0::handleUSBSuspend( void ) {
   //   PUTS("Suspend");
   if (deviceState.state != USBconfigured) {
 ...
 */
void Usb0::handleUSBResume( void ) {
   //   PUTS("Resume");
