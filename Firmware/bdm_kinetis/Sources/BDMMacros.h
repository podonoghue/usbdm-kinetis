...
inline uint8_t BDM_CMD_GO()                                         { return cmd_0_0(_BDM_GO);         }
//!< Target Go until (HC12)
inline uint8_t BDM12_CMD_GO_UNTIL()                                 { return cmd_0_0(_BDM12_GO_UNTIL); }
//!< Target Trace a single instruction (HC12,HCS08,RS08)
inline uint8_t BDM_CMD_TRACE1()                                     { return cmd_0_0(_BDM_TRACE1);     }
//!<  Target Go with tagging (HC12?)
inline uint8_t BDM_CMD_TAGGO()                                      { return cmd_0_0(_BDM_TAGGO);      }

// Write memory using X as a pointer with automatic pre-increment (HC12)
inline uint8_t BDM12_CMD_WRITE_NEXT(uint8_t value)                  { return cmd_1W_0(_BDM12_WRITE_NEXT,value); }

// Write register commands
//! Wite REG (HC12)
inline uint8_t BDM12_CMD_WRITE_REG(uint8_t reg, uint16_t value)     { return cmd_1W_0(_BDM12_WRITE_REG+reg, value); }
//! Wite PC (HC12)
inline uint8_t BDM12_CMD_WRITE_PC(uint16_t value)                   { return cmd_1W_0(_BDM12_WRITE_PC, value);      }
//! Write D(HC12)
inline uint8_t BDM12_CMD_WRITE_D(uint16_t value)                    { return cmd_1W_0(_BDM12_WRITE_D, value);       }
//! Write X (HC12)
inline uint8_t BDM12_CMD_WRITE_X(uint16_t value)                    { return cmd_1W_0(_BDM12_WRITE_X, value);       }
//! Write Y (HC12)
inline uint8_t BDM12_CMD_WRITE_Y(uint16_t value)                    { return cmd_1W_0(_BDM12_WRITE_Y, value);       }
//! Write SP (HC12)
inline uint8_t BDM12_CMD_WRITE_SP(uint16_t value)                   { return cmd_1W_0(_BDM12_WRITE_SP, value);      }

//! Read memory using X as a pointer with automatic pre-increment (HC12)
inline uint8_t BDM12_CMD_READ_NEXT(uint16_t *value_p)               { return cmd_0_1W(_BDM12_READ_NEXT, value_p);   }

// Read register commands
//! Read REG (HC12)
inline uint8_t BDM12_CMD_READ_REG(uint8_t reg, uint16_t *value_p)   { return cmd_0_1W(_BDM12_READ_REG+reg, value_p); }
//! Read PC (HC12)
inline uint8_t BDM12_CMD_READ_PC(uint16_t *value_p)                 { return cmd_0_1W(_BDM12_READ_PC, value_p);      }
//! Read D (HC12)
inline uint8_t BDM12_CMD_READ_D(uint16_t *value_p)                  { return cmd_0_1W(_BDM12_READ_D, value_p);       }
//! Read X (HC12)
inline uint8_t BDM12_CMD_READ_X(uint16_t *value_p)                  { return cmd_0_1W(_BDM12_READ_X, value_p);       }
//! Read Y (HC12)
inline uint8_t BDM12_CMD_READ_Y(uint16_t *value_p)                  { return cmd_0_1W(_BDM12_READ_Y, value_p);       }
//! Read SP (HC12)
inline uint8_t BDM12_CMD_READ_SP(uint16_t *value_p)                 { return cmd_0_1W(_BDM12_READ_SP, value_p);      }

// Hardware commands
//! Enable ACKN (HC12,HCS08,RS08)
inline uint8_t cmd_ACK_ENABLE()                                     { return cmd_0_0(_BDM_ACK_ENABLE);  }
//! Disable ACKN  (HC12,HCS08) - Does timeout on ACKN
inline uint8_t cmd_ACK_DISABLE()                                    { return cmd_0_0(_BDM_ACK_DISABLE); }
//! Halt Target (HC12,HCS08,RS08)
inline uint8_t cmd_BACKGROUND()                                     { return cmd_0_0(_BDM_BACKGROUND);  }

// Read and write commands
//! Write 16-bit value (HC12)
inline uint8_t BDM12_CMD_WRITEW(uint16_t addr, uint16_t value)      { return cmd_2W_0(_BDM12_WRITE_WORD, addr, value);   }
//! Write 8-bit value (HC12)
inline uint8_t BDM12_CMD_WRITEB(uint16_t addr, uint8_t value)       { return cmd_2WB_0(_BDM_WRITE_BYTE, addr, value);    }
//! Read 16-bit value (HC12)
inline uint8_t BDM12_CMD_READW(uint16_t addr,  uint16_t *value_p)   { return cmd_1W_1W(_BDM12_READ_WORD, addr, value_p); }
//! Read 8-bit value (HC12)
inline uint8_t BDM12_CMD_READB(uint16_t addr,  uint8_t *value_p)    { return cmd_1W_1WB(_BDM_READ_BYTE, addr, value_p);  }

// Read and writes from/to the BDM memory space
//! Write 16-bit value  BDM address space (HC12)
inline uint8_t BDM12_CMD_BDWRITEW(uint16_t addr, uint16_t value)    { return cmd_2W_0(_BDM12_WRITE_BD_WORD, addr, value);    }
//! Write 8-bit value  BDM address space (HC12)
inline uint8_t BDM12_CMD_BDWRITEB(uint16_t addr, uint8_t value)     { return cmd_2WB_0(_BDM12_WRITE_BD_BYTE, addr, value);   }
//! Read 16-bit value  BDM address space (HC12)
inline uint8_t BDM12_CMD_BDREADW(uint16_t addr,  uint16_t *value_p) { return cmd_1W_1W(_BDM12_READ_BD_WORD, addr, value_p);  }
//! Read 8-bit value  BDM address space(HC12)
inline uint8_t BDM12_CMD_BDREADB(uint16_t addr,  uint8_t *value_p)  { return cmd_1W_1WB(_BDM12_READ_BD_BYTE, addr, value_p); }
...
inline uint8_t BDM08_CMD_READB(uint16_t addr, uint8_t *value_p)                  { return cmd_1W_1B(_BDM_READ_BYTE, addr, value_p);            }
//! Write 8-bit memory value (HCS08,RS08)
inline uint8_t BDM08_CMD_WRITEB(uint16_t addr, uint8_t value)                    { return cmd_1W1B_0(_BDM_WRITE_BYTE, addr, value);            }
//! Write memory using ++H:X as a pointer
inline uint8_t BDM08_CMD_WRITE_NEXT(uint8_t value)                               { return cmd_1B_0(_BDM08_WRITE_NEXT, value);                  }
//! Read memory using ++H:X as a pointer
inline uint8_t BDM08_CMD_READ_NEXT(uint8_t *value_p)                             { return cmd_0_1B(_BDM08_READ_NEXT, value_p);                 }
...
// Read register commands
//! Read PC (HC08)
inline uint8_t BDM08_CMD_READ_PC(uint16_t *value_p)         { return cmd_0_1W(_BDM08_READ_PC, value_p);  }
//! Read SP (HC08)
inline uint8_t BDM08_CMD_READ_SP(uint16_t *value_p)         { return cmd_0_1W(_BDM08_READ_SP, value_p);  }
//! Read HX (HC08)
inline uint8_t BDM08_CMD_READ_HX(uint16_t *value_p)         { return cmd_0_1W(_BDM08_READ_HX, value_p);  }
//! Read A (HC08)
inline uint8_t BDM08_CMD_READ_A(uint8_t *value_p)           { return cmd_0_1B(_BDM08_READ_A, value_p);   }
//! Read CCR (HC08)
inline uint8_t BDM08_CMD_READ_CCR(uint8_t *value_p)         { return cmd_0_1B(_BDM08_READ_CCR, value_p); }
//! Read BKPT (HC08) - No ACK fix - pgo
inline void BDM08_CMD_READ_BKPT(uint16_t *value_p)          { cmd_0_1W_NOACK(_BDM08_READ_BKPT, value_p); }

// Write register commands
//! Write PC (HC08)
inline uint8_t BDM08_CMD_WRITE_PC(uint16_t value)           { return cmd_1W_0(_BDM08_WRITE_PC, value);    }
//! Write SP (HC08)
inline uint8_t BDM08_CMD_WRITE_SP(uint16_t value)           { return cmd_1W_0(_BDM08_WRITE_SP, value);    }
//! Write HX (HC08)
inline uint8_t BDM08_CMD_WRITE_HX(uint16_t value)           { return cmd_1W_0(_BDM08_WRITE_HX, value);    }
//! Write A (HC08)
inline uint8_t BDM08_CMD_WRITE_A(uint8_t value)             { return cmd_1B_0(_BDM08_WRITE_A, value);     }
//! Write CCR (HC08)
inline uint8_t BDM08_CMD_WRITE_CCR(uint8_t value)           { return cmd_1B_0(_BDM08_WRITE_CCR, value);   }
//! Write Breakpoint (HC08) - No ACK fix - pgo
inline void BDM08_CMD_WRITE_BKPT(uint16_t value)            { cmd_1W_0_NOACK(_BDM08_WRITE_BKPT, value);   }
//! Write Shadow PC (RS08)
inline uint8_t BDMRS08_CMD_WRITE_SPC(uint16_t value)        { return cmd_1W_0(_BDMRS08_WRITE_SPC, value); }
...
inline uint8_t BDMZ12_CMD_TRACE1()                          { return cmd_0_0(_BDMZ12_TRACE1); }

// Coldfire V1 Commands
//! Enable ACKN (CFv1)
inline uint8_t BDMCF_CMD_ACK_ENABLE()                       { return cmd_0_0(_BDMCF_ACK_ENABLE);  }
//! Disable ACKN (Expects ACK but times out) (CFv1)
inline uint8_t BDMCF_CMD_ACK_DISABLE()                      { return cmd_0_0(_BDMCF_ACK_DISABLE); }
//! Halt Target (CFv1)
inline uint8_t BDMCF_CMD_BACKGROUND()                       { return cmd_0_0(_BDMCF_BACKGROUND);  }
//! Target Go (CFv1)
inline uint8_t BDMCF_CMD_GO()                               { return cmd_0_0(_BDMCF_GO);          }
//! No Operation (CFv1)
inline uint8_t BDMCF_CMD_NOP()                              { return cmd_0_0(_BDMCF_NOP);         }
//! Sync PC (CFv1)
inline uint8_t BDMCF_CMD_SYNC_PC()                          { return cmd_0_0(_BDMCF_SYNC_PC);     }

//! Read Register (CFv1)
inline uint8_t BDMCF_CMD_READ_REG(uint8_t regNo, uint32_t  *value_p)    { return cmd_0_1L(_BDMCF_READ_Rn|(regNo),value_p);   }
//! Read Control Register (CFv1)
inline uint8_t BDMCF_CMD_READ_CREG(uint8_t regNo, uint32_t  *value_p)   { return cmd_0_1L(_BDMCF_READ_CREG|(regNo),value_p); }
//! Read Debug Register (CFv1)
inline uint8_t BDMCF_CMD_READ_DREG(uint8_t regNo, uint32_t  *value_p)   { return cmd_0_1L(_BDMCF_READ_DREG|(regNo),value_p); }
//! Read Trace buffer (CFv1)
inline uint8_t BDMCF_CMD_READ_PSTBe(uint8_t regNo, uint32_t  *value_p)  { return cmd_0_1L(_BDMCF_READ_PSTB|(regNo),value_p); }
...
//! Write Register (CFv1)
inline uint8_t BDMCF_CMD_WRITE_REG(uint8_t regNo, uint32_t value)       { return cmd_1L_0(_BDMCF_WRITE_Rn|(regNo),value);   }
//! Write Control Register (CFv1)
inline uint8_t BDMCF_CMD_WRITE_CREG(uint8_t regNo, uint32_t value)      { return cmd_1L_0(_BDMCF_WRITE_CREG|(regNo),value); }
//! Write Debug Register (CFv1)
inline uint8_t BDMCF_CMD_WRITE_DREG(uint8_t regNo, uint32_t value)      { return cmd_1L_0(_BDMCF_WRITE_DREG|(regNo),value); }
...
//inline uint8_t BDMCF_CMD_READ_MEM_S(uint32_t addr24, uint8_t *value_p) { return cmd_1A_CS_1B(_BDMCF_READ_MEM_WS|_BDMCF_SZ_BYTE,addr24,value_p); }

//! Read 8-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_READ_MEM_B(uint32_t addr24, uint8_t *value_p)  { return cmd_1A_1B(_BDMCF_READ_MEM|_BDMCF_SZ_BYTE,addr24,value_p); }
////! Read 16-bit memory value (CFv1)
//inline uint8_t BDMCF_CMD_READ_MEM_W(uint32_t addr24, uint16_t *value_p) { return cmd_1A_1W(_BDMCF_READ_MEM|_BDMCF_SZ_WORD,addr24,value_p); }
////! Read 32-bit memory value (CFv1)
//inline uint8_t BDMCF_CMD_READ_MEM_L(uint32_t addr24, uint32_t *value_p) { return cmd_1A_1L(_BDMCF_READ_MEM|_BDMCF_SZ_LONG,addr24,value_p); }

//! Read consecutive 8-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_DUMP_MEM_B(uint8_t *value_p)                   { return cmd_0_1B(_BDMCF_DUMP_MEM|_BDMCF_SZ_BYTE, value_p); }
//! Read consecutive 16-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_DUMP_MEM_W(uint16_t *value_p)                  { return cmd_0_1W(_BDMCF_DUMP_MEM|_BDMCF_SZ_WORD, value_p); }
//! Read consecutive 32-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_DUMP_MEM_L(uint32_t *value_p)                  { return cmd_0_1L(_BDMCF_DUMP_MEM|_BDMCF_SZ_LONG, value_p); }

//! Write 8-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_WRITE_MEM_B(uint32_t addr24, uint8_t value)    { return cmd_1A1B_0(_BDMCF_WRITE_MEM|_BDMCF_SZ_BYTE, addr24, value); }
//! Write 16-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_WRITE_MEM_W(uint32_t addr24, uint16_t value)   { return cmd_1A1W_0(_BDMCF_WRITE_MEM|_BDMCF_SZ_WORD, addr24, value); }
//! Write 32-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_WRITE_MEM_L(uint32_t addr24, uint32_t value)   { return cmd_1A1L_0(_BDMCF_WRITE_MEM|_BDMCF_SZ_LONG, addr24, value); }

//! Write consecutive 8-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_FILL_MEM_B(uint8_t value)                      { return cmd_1B_0(_BDMCF_FILL_MEM|_BDMCF_SZ_BYTE, value); }
//! Write consecutive 16-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_FILL_MEM_W(uint16_t value)                     { return cmd_1W_0(_BDMCF_FILL_MEM|_BDMCF_SZ_WORD, value); }
//! Write consecutive 32-bit memory value (CFv1)
inline uint8_t BDMCF_CMD_FILL_MEM_L(uint32_t value)                     { return cmd_1L_0(_BDMCF_FILL_MEM|_BDMCF_SZ_LONG, value); }
